	...
	$(document).on('submit', '#choosecomparerform', function (e) {
		...
		if(grants.length == 1)
		{
			...
			for (i= 0; i<grants.length; i++) {
				...
				grant.apersonmonths = $tr.find("td.apmonthnum").text();
				grant.cpersonmonths = $tr.find("td.cpmonthnum").text();
				grant.spersonmonths = $tr.find("td.spmonthnum").text();
				grant.description = comparisons[grants[i]];
				grant.awardperiod1 = $tr.find("td.fromdate").text();
				grant.awardperiod2 = $tr.find("td.todate").text();
